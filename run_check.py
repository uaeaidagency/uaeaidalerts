...
try:
    ...
    _LIVE_EVENTS_AVAILABLE = True
except Exception:
    _LIVE_EVENTS_AVAILABLE = False


def _send_live_event_alert(
    ev: dict,
    recipients: List[str],
    tg_chat_ids: List,
    tg_enabled: bool,
    ...
    """Email + Telegram alert for a brand-new major live event (e.g. Venezuela
    twin earthquakes today). Bypasses the curated CSV pipeline so events in
    countries NOT on the curated list still reach leadership immediately."""
    country = ev.get("country", "Unknown")
    label   = ev.get("type_label", "Disaster")
    name    = ev.get("name", label)
    alert   = ev.get("alert", "").upper() or "ACTIVE"
    date    = ev.get("date", "")
    url     = ev.get("url", "")
    mag     = ev.get("magnitude", "")
    src     = ev.get("source", "GDACS")

    subject = f"[UAE Aid · EMERGENCY EVENT · {alert}] {country} — {label}"
...
<table cellpadding="0" cellspacing="0" style="background:#1D252C;color:white;
       padding:14px 18px;border-radius:8px;border-top:3px solid #B81D24;
       border-bottom:3px solid #FBAE40;width:100%;max-width:660px;">
  <tr><td>
    <div style="font-weight:700;letter-spacing:.5px;">UAE AID AGENCY</div>
    <div style="font-size:12px;color:#FFB3B8;">🚨 EMERGENCY EVENT — IMMEDIATE REVIEW</div>
  </td></tr>
</table>
<p style="font-size:18px;margin:18px 0 6px 0;">
  <b style="color:#B81D24;">{label} — {country}</b>
</p>
<p style="font-size:13px;color:#374151;margin:0 0 14px 0;">
  Detected by <b>{src}</b> · Alert level <b>{alert}</b>{(' · Date ' + date) if date else ''}{(' · M' + mag) if mag else ''}
</p>
<div style="background:#F4F7FA;border-left:4px solid #B81D24;padding:12px 14px;
            border-radius:6px;font-size:13px;line-height:1.5;">
  <b>Event:</b> {name}<br><br>
  <b>Why this matters:</b> This is a major active disaster picked up directly
  from the {src} live feed, regardless of whether the country is on the UAE
  curated priority list. Leadership should review immediately for response
  posture, partner engagement, and pledge consideration.
</div>
{('<p style="margin-top:14px;font-size:13px;"><a href="' + url + '">View the official event report ↗</a></p>') if url else ''}
{dashboard_button or ""}
<p style="font-size:11px;color:#9CA3AF;margin-top:18px;border-top:1px solid #E5E7EB;padding-top:10px;">
  Generated automatically by the UAE Aid Agency Tier-2+ Alert Agent (live event detector).
  Source: GDACS multi-hazard alert feed + ReliefWeb current disasters RSS.
</p>
</body>
</html>"""

    tg_body = (
        f"🚨 <b>UAE AID AGENCY</b> — Emergency Event\n\n"
        f"<b>{country} — {label}</b>\n"
        f"<i>{src} · alert level {alert}{(' · ' + date) if date else ''}{(' · M' + mag) if mag else ''}</i>\n\n"
        f"<b>{name}</b>\n\n"
        f"This is a major active disaster from live feeds. Leadership should "
        f"review immediately for response posture."
    )
    if url:
        tg_body += f"\n\n🔗 <a href=\"{url}\">Official event report</a>"
    if dashboard_link:
        tg_body += dashboard_link

    email_ok = True
    tg_ok = True
    if recipients:
        try:
            email_ok = send_alert(recipients, subject, body_html)
            if email_ok:
                _log(f"  Live event email sent ({country} · {label}) to {recipients}")
            else:
                _log(f"  ERROR: live event email failed ({country} · {label})")
        except Exception as e:
            _log(f"  ERROR: live event email exception: {e}")
            email_ok = False
    if tg_enabled and tg_chat_ids:
        try:
            tg_ok = telegram_sender.send_alert(tg_chat_ids, tg_body)
            if tg_ok:
                _log(f"  Live event Telegram sent ({country} · {label}) to {tg_chat_ids}")
            else:
                _log(f"  ERROR: live event Telegram failed ({country} · {label})")
        except Exception as e:
            _log(f"  ERROR: live event Telegram exception: {e}")
            tg_ok = False
...
def _load_state() -> Optional[Dict[str, int]]:
    ...
    try:
        ...
        # A corrupt state file (e.g. leftover Git merge-conflict markers) must
        # NEVER crash the whole agent. Log it, quarantine the bad file, and
        # treat this as a first run (seed fresh state, send no tier alerts).
        try:
            bad = STATE_FILE + ".corrupt"
            os.replace(STATE_FILE, bad)
            _log(f"WARN: state file was unreadable ({e}); moved to {os.path.basename(bad)} and reseeding.")
        except Exception:
            _log(f"WARN: state file unreadable ({e}); reseeding.")
        return None
...
def run(force: bool = False, dry_run: bool = False, simulate: Optional[Dict[str, int]] = None) -> int:
    ...
    # ── Live emergency events (GDACS Red/Orange + ReliefWeb current) ──
    # The curated CSV scoring engine only sees countries on the manually
    # maintained list. A brand-new major natural disaster in a country NOT
    # on that list (e.g. Venezuela earthquakes, Madagascar cyclone) would
    # never trigger an alert. This block closes that gap — it pulls live
    # disaster feeds, identifies new events, and fires a Tier-1 alert.
    live_events_fired = 0
    if not dry_run and not simulate and _LIVE_EVENTS_AVAILABLE:
        try:
            new_events = live_events.detect_new_major_events()
        except Exception as e:
            _log(f"  WARN: live event detection failed: {e}")
            new_events = []
        # Only Red-level GDACS or ReliefWeb "current" disasters get an
        # alert (Orange goes to the digest as a heads-up).
        major = [e for e in new_events if e.get("alert", "").lower() in ("red", "current")]
        if major:
            _log(f"Detected {len(major)} new major live event(s) — firing alerts.")
        delivered_events: List[dict] = []
        for ev in major[:5]:   # cap to 5 per run to avoid floods
            try:
                ok = _send_live_event_alert(
                    ev, recipients, tg_chat_ids, tg_enabled,
                ...
                _log(f"  ERROR sending live event alert ({ev.get('id')}): {e}")
        if delivered_events:
            try:
                live_events.mark_seen(delivered_events)
            except Exception as e:
                _log(f"  WARN: could not persist seen live events: {e}")

    # ── World news digest ────────────────────────────────────────────
    # When NO crisis tier change AND no live-event alert this cycle, still
    # send leadership a snapshot of the latest worldwide news so the
    # 30-minute cadence always delivers something useful instead of silence.
    if (not dry_run and not simulate
            and len(alerts) == 0 and live_events_fired == 0
            and _NEWS_DIGEST_AVAILABLE):
        try:
            news_digest.send_world_news_digest(
                recipients, tg_chat_ids, tg_enabled,
                dashboard_button=dashboard_button,
                dashboard_link=dashboard_link,
                logger=_log,
            )
        except Exception as e:
            _log(f"  WARN: world news digest failed: {e}")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    p.add_argument(
        "--news-digest",
        action="store_true",
        help="Send the world news digest immediately (ignores tier state). "
             "Useful for verifying the 30-minute news email end-to-end.",
    )
    ...
    if args.news_digest:
        ...
        if not _NEWS_DIGEST_AVAILABLE:
            print("news_digest module unavailable.")
            return 1
        ok = news_digest.send_world_news_digest(
            recipients, tg_chat_ids, tg_enabled,
            dashboard_button=dashboard_button, dashboard_link=dashboard_link,
            logger=_log,
        )
        print("Done. News digest sent." if ok else "News digest not sent (see log).")
